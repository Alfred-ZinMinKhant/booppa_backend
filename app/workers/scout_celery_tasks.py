...
from app.services.email_layout import branded_email_html, email_button, is_adv_subject
...
def _outreach_compliance_error(subject: str, body_html: str, to_email: str) -> str | None:
    """Return why this message must not be sent, or None if it may go out.

    A last gate in front of the SMTP call, checking the rendered artefact
    rather than the code that produced it. It exists for two reasons:

    1. Rows APPROVED before 2026-08-07 carry bodies generated by the old
       non-compliant templates — no "<ADV>", no in-body unsubscribe, and the
       literal "Dear [Name],". Those rows are still sitting in APPROVED and
       would have gone out unchanged on the first run after the beat entry is
       re-enabled. Fixing the generator alone does not reach them.
    2. It fails closed on the postal address, which has no safe default.

    Failures here are permanent, not transient: they mark the prospect
    SEND_FAILED with the reason so a human regenerates the body rather than
    the task retrying an identically-defective message.
    """
    if not is_adv_subject(subject):
        return 'subject missing "<ADV>" prefix (Spam Control Act Sch 2 para 3)'
    if not (settings.COMPANY_POSTAL_ADDRESS or "").strip():
        return "COMPANY_POSTAL_ADDRESS is unset — cannot include the sender address"
    if (settings.COMPANY_POSTAL_ADDRESS or "").strip() not in (body_html or ""):
        return "body missing the sender postal address (Sch 2 para 2(b))"
    if unsubscribe_url(to_email) not in (body_html or ""):
        return "body missing an in-body unsubscribe link for this recipient (Sch 2 para 2(a))"
    if "[Name]" in (body_html or ""):
        return "body still contains the unfilled [Name] placeholder"
    return None
...
def scout_send_approved_batch_task(self):
    ...
    """
    if not (settings.COMPANY_POSTAL_ADDRESS or "").strip():
        # Abort the whole batch rather than walking it and marking every row
        # SEND_FAILED: this is one missing setting, not a defect in any
        # prospect, and burning the queue over it would need a manual reset of
        # every row once the address is configured.
        logger.error(
            "[SCOUT send] aborted — COMPANY_POSTAL_ADDRESS is unset, so no "
            "advertisement can carry the sender address the Spam Control Act "
            "(Cap. 311A Sch 2 para 2(b)) requires. Nothing sent, nothing marked failed."
        )
        return
    ...
    try:
        ...
        for prospect in batch:
            ...
            # Subject fallback removed: a generic "<display_name> — Booppa"
            # would bypass the <ADV> prefix. A prospect with no stored subject
            # is a generation failure, not something to paper over at send time.
            subject = prospect.outreach_subject or ""
            problem = _outreach_compliance_error(subject, prospect.outreach_body_html, to_email)
            if problem:
                prospect.status = "SEND_FAILED"
                prospect.status_reason = f"blocked, not compliant: {problem}"[:500]
                logger.error(
                    "[SCOUT send] blocked prospect %s — %s", prospect.id, problem
                )
                failed += 1
