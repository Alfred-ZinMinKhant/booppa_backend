...
def compare_vendors(db: Session, vendor_ids: list[str]) -> dict:
    ...
    for vid in vendor_ids:
        ...
        vendors.append({
            ...
            "scores": {
                ...
                "procurement_interest_score": score.procurement_interest_score if score else 0,
            } if score else None,
            "status": {
                "verification_depth": status.verification_depth if status else "UNVERIFIED",
                "monitoring_activity": status.monitoring_activity if status else "NONE",
            ...
            "verification": {
                ...
                "verified_at": verify.verified_at.isoformat() if verify and hasattr(verify, 'verified_at') and verify.verified_at else None,
            } if verify else None,
    ...
    dimensions = [
        "total_score", "compliance_score", "visibility_score",
        "verification_depth", "risk_signal", "procurement_readiness",
        "confidence_score",
    ]
...
def find_comparable_vendors(db: Session, vendor_id: str, limit: int = 5) -> list[dict]:
    ...
    vendor_sectors = db.query(VendorSector).filter(VendorSector.vendor_id == vendor_id).all()
    if not vendor_sectors:
        return []

    sector_names = [s.sector for s in vendor_sectors]
    vendor_score = db.query(VendorScore).filter(VendorScore.vendor_id == vendor_id).first()
