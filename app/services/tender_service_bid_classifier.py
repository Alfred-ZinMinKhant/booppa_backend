...
def classify_tender(tender: dict, vendor_history: dict | None = None) -> dict:
    ...
    # No synthetic score "noise": the composite above is built from real tender
    # attributes (deadline, size fit, keywords, agency competition). Fabricating a
    # ±20% hash jitter made otherwise-identical tenders flip labels arbitrarily —
    # dishonest and non-reproducible. Tenders with identical real inputs get the
    # same label, which is correct.
