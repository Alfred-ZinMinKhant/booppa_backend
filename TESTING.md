...
| `TEST_AWS_ACCESS_KEY_ID` | backend | IAM user scoped to `booppa-reports-04bd50c4/reports/test/*` only — see "Real-AWS S3 setup" below. Used by tests that opt into the `real_s3` fixture. |
...
bucket under a `reports/test/<run-id>/` prefix and delete them on teardown.
(The `reports/` part comes from `S3Service.upload_pdf` which always prefixes
keys that way; we add `test/<run-id>/` inside it.) To make
that safe:
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::booppa-reports-04bd50c4/reports/test/*"
    }
  ]
}
```
...
that no `reports/test/<run-id>/...` keys remain after the run.
